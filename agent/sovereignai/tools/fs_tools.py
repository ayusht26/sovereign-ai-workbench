...
class FsRead(Tool):
    ...
    categories = ["general", "coding", "vision", "spreadsheet", "document_qa", "planning"]
    json_schema = {
        "type": "object",
        "properties": {
    ...
    def run(self, path: str, start_line: int | None = None, end_line: int | None = None) -> ToolResult:
        ...
        if p is None:
            return ToolResult.fail(f"Path '{path}' is outside the workspace root. Access denied.")
        if not p.exists():
        ...
        try:
            ...
            if start_line or end_line:
                ...
                lines = lines[s:e]
            content = "".join(lines)
            return ToolResult.ok({"content": content, "lines": len(lines), "path": str(p)})
...
class FsWrite(Tool):
    name = "fs_write"
    description = "Write or overwrite a file within the workspace. Creates parent directories as needed."
...
class FsList(Tool):
    ...
    def run(self, path: str = ".", recursive: bool = False) -> ToolResult:
        p = _validate_path(path)
        if p is None:
            return ToolResult.fail(f"Path '{path}' is outside the workspace root.")
        if not p.exists():
        ...
        return ToolResult.ok({"entries": entries, "count": len(entries), "directory": str(p)})
...
class FsGlob(Tool):
    ...
    def run(self, pattern: str, base: str = ".") -> ToolResult:
        base_path = _validate_path(base)
        if base_path is None:
            return ToolResult.fail(f"Base path '{base}' outside workspace.")
        ...
        return ToolResult.ok({"matches": matches, "count": len(matches), "pattern": pattern})
