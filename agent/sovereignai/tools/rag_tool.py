"""
rag_tool.py — Local knowledge base search via Qdrant.

Returns top-k chunks with source filenames and scores.
Role-based access control is enforced by the knowledge-base store.
"""
...
class RagSearch(Tool):
    ...
    def run(
        ...
        try:
            ...
            # Role is already resolved from the authenticated session.
            ...
            if not results:
                return ToolResult.ok({
                    ...
                    "message": (
                        ...
                        "Add documents with: sovai kb add <path>"
                    ),
                })
